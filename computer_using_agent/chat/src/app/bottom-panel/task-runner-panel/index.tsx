...
export function TaskRunnerPanel({ onScreenshotAdded, onSubmit, onTaskStart, onTaskComplete, panelMode: externalPanelMode, onModeChange, onThought, onAction, onClearThoughts, onClearActions, onAgentStarting }: TaskRunnerPanelProps) {
  ...
  // Clean up browser and state between tasks
  const cleanupBetweenTasks = async () => {
    try {
      ...
      // Add a longer delay to ensure cleanup is complete before next task
      console.log('Waiting for cleanup to complete...');
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      // Process next task in queue if in batch run mode
      if ((isBatchRun || autoRunning) && taskQueue.length > 0) {
        ...
        if (nextTask) {
          ...
          if (taskQueue.length > 1) {
            setTimeout(() => {
              // Trigger cleanup again to process next task
              cleanupBetweenTasks();
            }, 100);
          }
        }
      } else if ((isBatchRun || autoRunning) && taskQueue.length === 0) {
        console.log('🔥 QUEUE PROCESSING: All tasks completed, returning to task runner panel');
        setIsBatchRun(false);
      ...
      console.error('Error during cleanup between tasks:', error);
    }
  };
  ...
  const executeTaskWithVariables = async (task: Task, variables?: Record<string, string>) => {
    ...
    try {
        ...
        switch (event.type) {
          ...
          case 'tool_use':
            ...
            if (onAction && event.tool_name) {
              ...
              } else if (event.tool_name === 'js_inspector') {
                description = 'Executing JavaScript';
                details = 'Executing JavaScript code';
          ...
          case 'tool_result':
            ...
            // Handle js_inspector tool results
            ...
            if (event.output || event.error) {
              // Don't show js_inspector tool results to user (they're instructions for the agent)
            }
            break;
          case 'error':
      ...
      if (streamingMessageRef.current) {
        ...
        if (isBatchRun || autoRunning) {
          ...
          setTaskQueue(currentQueue => {
            const remainingQueue = currentQueue.filter(t => t.id !== task.id);
            
            console.log('🔥 QUEUE UPDATE:', {
              originalLength: currentQueue.length,
              remainingLength: remainingQueue.length,
              completedTaskId: task.id,
              nextTaskId: remainingQueue[0]?.id || 'none'
            });
            ...
            if (remainingQueue.length > 0) {
              ...
              console.log('🔥 SCHEDULING NEXT TASK:', nextTask.id);
              runAllLogger.logTaskExecution('executeTaskWithVariables-success', 'scheduling next task', nextTask.id);
      ...
      // Clean up browser and state before next task
      try {
        await cleanupBetweenTasks();
        runAllLogger.debug('executeTaskWithVariables-finally', 'Cleanup between tasks completed');
      } catch (cleanupError) {
      ...
      // Continue with next task if this is a batch run or auto-run
      if (isBatchRun || autoRunning) {
        runAllLogger.info('executeTaskWithVariables-completion', 'Processing next task in sequence');
        ...
        try {
          runAllLogger.info('executeTaskWithVariables-completion', 'Current queue before processing next', {
            queueLength: taskQueue.length,
            queueTaskIds: taskQueue.map(t => t.id),
            completedTaskId: task.id
          });
          
          // FIX: Filter out the completed task and get the remaining queue
          const remainingQueue = taskQueue.filter(t => t.id !== task.id);
          
          runAllLogger.info('executeTaskWithVariables-completion', 'Removing completed task from queue', {
            originalQueueLength: taskQueue.length,
            remainingQueueLength: remainingQueue.length,
            completedTaskId: task.id,
            nextTaskId: remainingQueue[0]?.id || 'none'
          });
          
          // Update the queue by removing the completed task
          setTaskQueue(remainingQueue);
          
          if (remainingQueue.length > 0) {
            // Get next task from remaining queue
            const nextTask = remainingQueue[0];
            
            runAllLogger.logTaskExecution('executeTaskWithVariables-completion', 'moving to next task', nextTask.id);
            
            // FIX: Check if we're already processing to prevent duplicates
            if (isProcessingNextTask) {
              runAllLogger.warn('executeTaskWithVariables-completion', 'Already processing next task, skipping', {
                nextTaskId: nextTask.id
              });
              return;
            }
            
            // Show "Agent starting" visual screen
            setIsAgentStarting(true);
            onAgentStarting?.(true);
            
            // Small delay to show "Agent starting" message
            setTimeout(async () => {
              setIsAgentStarting(false);
              onAgentStarting?.(false);
              
              runAllLogger.info('executeTaskWithVariables-completion', 'Executing next task after delay', {
                nextTaskId: nextTask.id
              });
              
              // FIX: Set flag to prevent conflicts and execute next task
              setIsProcessingNextTask(true);
              setIsAnyTaskRunning(true); // Ensure the running flag is set for next task
              await executeTaskWithVariables(nextTask, taskVariables);
              setIsProcessingNextTask(false);
            }, 1000);
          } else {
            // No more tasks in queue - auto-run complete
            runAllLogger.info('executeTaskWithVariables-completion', 'Auto-run complete: All tasks in queue processed');
            ...
            // Show task panel when run all completes
            setPanelMode('maximized');
            
            // Show first completed task report if available
            if (completedTaskForReport) {
              setTimeout(() => {
                setSelectedTaskReport(completedTaskForReport);
              }, 500);
            ...
            // Queue is already empty, just clean up state
            runAllLogger.info('executeTaskWithVariables-completion', 'Auto-run sequence completed successfully');
          }
        } catch (error) {
          runAllLogger.error('executeTaskWithVariables-completion', 'Error processing next task', error);
          ...
          // Show task panel when auto-run fails
          setPanelMode('maximized');
          
          // Reset task queue for next run all
          clearTaskQueue();
        }
      } else {
        // Single task execution completed - clear running flag and show report modal
        runAllLogger.info('executeTaskWithVariables-completion', 'Single task execution completed');
        setIsAnyTaskRunning(false);
        if (completedTaskForReport) {
          setTimeout(() => {
            setSelectedTaskReport(completedTaskForReport);
  ...
  const handleAllTasksVariables = async (variables: Record<string, string>) => {
    ...
    // FIX: Use pendingTasks directly to avoid race condition with taskQueue
  ...
  const startAutoRun = async () => {
    ...
    if (allVariables.length > 0) {
      ...
      // FIX: Use pendingTasks directly instead of taskQueue to avoid race condition
      setAutoRunning(true);
      const firstTask = pendingTasks[0]; // Use pendingTasks instead of potentially stale taskQueue
      if (firstTask) {
        runAllLogger.logTaskExecution('startAutoRun', 'starting first task', firstTask.id);
        await executeTaskWithVariables(firstTask);
      } else {
        runAllLogger.error('startAutoRun', 'No first task found despite having pending tasks');
        setAutoRunning(false);
      }
    }
  };
  ...
  const saveSteps = async (taskId: string, steps: string[]) => {
    try {
      ...
      if (response.ok) {
        ...
        setTasks(prevTasks => 
          prevTasks.map(t => 
            t.id === taskId ? { ...t, instructions: steps } : t
          )
        );
      } else {
  ...
  // Create new task
  const createTask = async (instructions: string[]) => {
    try {
      const response = await fetch(`${API_BASE_URL}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const statusConfig = {
    pending: { color: 'bg-yellow-500', textColor: 'text-gray-300', label: 'PENDING' },
        ...
        {panelMode !== 'minimized' && (
          <div className="p-4 border-b border-zinc-600">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                {(taskQueue.length > 0 || isAnyTaskRunning) ? 'Running...' : 'Run all'}
              </button>
              {(taskQueue.length > 0 || isAnyTaskRunning) && (
                <button
                  onClick={stopAutoRun}
              ...
              <button
                onClick={() => runAllLogger.exportLogs()}
            ...
            {/* Debug Panel for Queue */}
            {(taskQueue.length > 0 || isAnyTaskRunning) && (
              <div className="mt-3 p-3 bg-zinc-800 rounded border border-zinc-600">
                <h4 className="text-sm font-medium text-zinc-300 mb-2">🔍 Queue Debug</h4>
                <div className="text-xs text-zinc-400 space-y-1">
                  <div>Queue Length: <span className="text-yellow-400 font-mono">{taskQueue.length}</span></div>
                  <div>isAnyTaskRunning: <span className="text-yellow-400 font-mono">{isAnyTaskRunning.toString()}</span></div>
                  <div>Current Task: <span className="text-yellow-400 font-mono">{currentTask?.id || 'none'}</span></div>
                  {taskQueue.length > 0 && (
                    <div>
                      <div className="text-zinc-300 font-medium mb-1">Queue Items:</div>
                        ...
                        {taskQueue.map((taskId, index) => {
                          ...
                          return (
                            <div key={taskId} className="flex items-center gap-2">
                              ...
                              <span className="text-yellow-400 font-mono text-xs">{taskId.substring(0, 8)}...</span>
                              <span className="text-zinc-400">({task?.status || 'unknown'})</span>
                              {index === 0 && <span className="text-green-400 text-xs">← next</span>}
                            </div>
                          );
                        })}
                      </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {tasks.map((task) => (
                    <tr
                      key={task.id}
                      className={`border-b border-zinc-700 hover:bg-blue-900/30 transition-colors ${
